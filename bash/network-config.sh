...
########################################
#commands are listed in which hostname , network interface name , lan address, lan hostname,external ip address and external ip address name are stored to respective variables.


hostname=$(hostname)
networkinterface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lanaddress=$(ip a s $networkinterface | awk '/inet /{gsub(/\/.*/,"");print $2}')
...
######## Task 2
#commands are listed in which ip adress and router hostname are stored into a variable.

routerip=$(ip r | awk '/via /{gsub(/\/.*/,"");print $3}')
routername=$(getent hosts $routerip | awk '{print $2}')

#commands are entered to display the hostname, network interface name, lan address, lan hostname,#external ip address  ,external name , router ip address and routerhostname
cat <<EOF
Hostname        : $hostname
